...
def invoke_agent(session_id, prompt, enable_trace=True):
    ...
    try:
        ...
        # The response from invoke_agent is an EventStream
        # We need to iterate through it to get the completion
        try:
        ...
        # If we still don't have a completion, log an error but don't use hardcoded response
        if not completion:
            ...
            # Return empty completion instead of hardcoded message
            completion = ""
